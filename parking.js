export class Parking {
  constructor() {
    ...
    var parkingLot = this.initializeSlots(this.ROWS, this.COLS);
  ...
  randomizeTypeInSlots(parkingSlot) {
    var parkingSlotWithRandomizedTypes = parkingSlot;
    for (var i = 0; i < this.ROWS; i++) {
      for (var j = 0; j < this.COLS; j++) {
        var randomTypeGenerator = Math.floor(Math.random() * 10);
  ...
  parkVehicle(size, entrance, plateNumber) {
    ...
    var parkPosition = this.whereIsNearestSlotFromEntrance(
      size,
      entrance,
      plateNumber
    );
